...
const update = (data) => {
  //when clicking the buttons only that activity data will show up
  ...
  graph
    ...
    .on('mouseover', (d, i, n) => {
      ...
      //set x dotted line coords (x1, x2, y1, y2) which is the vertical line
      xDottedline
        .attr('x1', x(new Date(d.date)))
        .attr('x2', x(new Date(d.date)))
        .attr('y1', graphHeight)
        .attr('y2', y(d.distance))
      //set y dotted line coords (x1, x2, y1, y2) which is the horizontal line
      yDottedLine
        .attr('x1', 0)
        .attr('x2', x(new Date(d.date)))
